...
from codedocumentation.src.be.agent_tools.deps import CodeDocumentationDeps
from codedocumentation.src.be.agent_tools.read_code_components import read_code_components_tool
from codedocumentation.src.be.agent_tools.str_replace_editor import str_replace_editor_tool
from codedocumentation.src.be.llm_services import create_fallback_models
from codedocumentation.src.be.prompt_template import SYSTEM_PROMPT, LEAF_SYSTEM_PROMPT, format_user_prompt
from codedocumentation.src.be.utils import is_complex_module, count_tokens
from codedocumentation.src.be.cluster_modules import format_potential_core_components
from codedocumentation.src.config import MAX_TOKEN_PER_LEAF_MODULE
...
async def generate_sub_module_documentation(
    ctx: RunContext[CodeDocumentationDeps],
    ...
    for sub_module_name, core_component_ids in sub_module_specs.items():
        ...
        if is_complex_module(ctx.deps.components, core_component_ids) and ctx.deps.current_depth < ctx.deps.max_depth and num_tokens >= MAX_TOKEN_PER_LEAF_MODULE:
            sub_agent = Agent(
                ...
                deps_type=CodeDocumentationDeps,
