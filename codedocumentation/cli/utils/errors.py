...
class CodeDocumentationError(Exception):
    """Base exception for CodeDocumentation CLI errors."""
...
class ConfigurationError(CodeDocumentationError):
...
class APIError(CodeDocumentationError):
...
class FileSystemError(CodeDocumentationError):
...
def handle_error(error: Exception, verbose: bool = False) -> int:
    ...
    """
    if isinstance(error, CodeDocumentationError):
