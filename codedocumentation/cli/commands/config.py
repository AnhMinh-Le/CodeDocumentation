"""
Configuration commands for CodeDocumentation CLI.
"""
...
from codedocumentation.cli.config_manager import ConfigManager
from codedocumentation.cli.utils.errors import (
    ConfigurationError, 
    handle_error, 
    EXIT_SUCCESS,
    EXIT_CONFIG_ERROR
)
from codedocumentation.cli.utils.validation import (
    validate_url,
    validate_api_key,
    validate_model_name,
    is_top_tier_model,
...
def config_group():
    """Manage CodeDocumentation configuration (API credentials and settings)."""
    pass
...
def config_set(
    ...
    """
    Set configuration values for CodeDocumentation.
    ...
    $ codedocumentation config set --api-key sk-abc123 --base-url https://api.anthropic.com \\
        --main-model claude-sonnet-4 --cluster-model claude-sonnet-4
    
    \b
    # Update only API key
    $ codedocumentation config set --api-key sk-new-key
    """
    try:
...
def config_show(output_json: bool):
    ...
    \b
    # Display as JSON
    $ codedocumentation config show --json
    """
    try:
        manager = ConfigManager()
        ...
        if not manager.load():
            ...
            click.echo("\nPlease run 'codedocumentation config set' to configure your API credentials:")
            click.echo("  codedocumentation config set --api-key <key> --base-url <url> \\")
            click.echo("    --main-model <model> --cluster-model <model>")
            click.echo("\nFor more help: codedocumentation config set --help")
            sys.exit(EXIT_CONFIG_ERROR)
        ...
        if output_json:
            ...
            click.echo()
            click.secho("CodeDocumentation Configuration", fg="blue", bold=True)
            click.echo("━" * 40)
            click.echo()
...
def config_validate(quick: bool, verbose: bool):
    ...
    $ codedocumentation config validate --quick
    
    \b
    # Verbose output
    $ codedocumentation config validate --verbose
    """
    try:
        click.echo()
        ...
        if not manager.load():
            ...
            click.echo()
            click.echo("Error: Configuration is incomplete. Run 'codedocumentation config set --help' for setup instructions.")
            sys.exit(EXIT_CONFIG_ERROR)
        ...
        if not api_key:
            ...
            click.echo()
            click.echo("Error: API key not set. Run 'codedocumentation config set --api-key <key>'")
            sys.exit(EXIT_CONFIG_ERROR)
